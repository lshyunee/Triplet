...
const ProgressBar = styled.div<{ paid: string; }>`
    width: ${(props) => props.paid || '0%'}; /* 30% 진행률 */
    height: 100%;
    border-radius: 50px;
...
const SnsTravelDetailCard: React.FC<TravelDetailCardProps> = 
        ({title, startDate, endDate, country, memberCount,usedBudget,totalBudget,percentage, currency}) => {
    ...
    return (
        <PositionDiv>
            <CardDiv>
                ...
                <ProgressContainer>
                    <ProgressBar paid={String(percentage).toString()} />
                </ProgressContainer>
                <PriceInfo>
                    <PriceUsedP>
                        {usedBudget} 
...
export default SnsTravelDetailCard;
