...
const ExchangePage = () => {
  ...
  const fromOnChange = (e: React.FormEvent<HTMLInputElement>) => {
    ...
    if (travelWalletDetailData && numericValueAsNumber > travelWalletDetailData?.data?.balance) {
  ...
  const handleRecharge = async () => {
    if (!isInvalid && !isBalanceExceeded) {
      ...
      try {
        // POST 요청을 보내서 반환
        await axiosInstance.post('/travel-wallet/return', requestBody); // axiosInstance를 사용하여 POST 요청

        // 모달 열기
        setIsModalOpen(true);

      } catch (error) {
        console.error('반환 중 오류 발생:', error);
      }
    }
  };
  ...
  return (
      <>
        ...
        <s.Container>
          <s.TitleArea>
            <s.Title>
              여행 통장 잔액 : {travelWalletDetailData?.data?.balance}{' '}
              {travelWalletDetailData?.data?.currency}
            </s.Title>
          </s.TitleArea>
          <s.Caption>반환 금액 입력</s.Caption>
          <s.InputArea>
            <s.ExchangeInput onChange={fromOnChange} value={fromAmount}/>
          </s.InputArea>
          {/* 잔액 부족 메시지 표시 */}
          {isBalanceExceeded && <s.ErrorMessage>잔액이 부족합니다</s.ErrorMessage>}
          <s.Bottom>* 내 외화 계좌로 반환됩니다.</s.Bottom>
          <s.BtnArea>
        ...
        {isModalOpen && (
            <s.ModalOverlay>
              <s.ModalContainer>
                <s.ModalText>반환이 완료되었습니다.</s.ModalText>
